<template>
  <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 sticky top-0 z-40 shadow-sm">
    ...
    <!-- Espacio vacío a la derecha para mantener el centrado del logo -->
    <div class="w-24 lg:w-48"></div>
...
import { useAuth } from '~/composables/useAuth'; // Importamos el composable correcto

const { user, logout } = useAuth(); // Obtenemos 'user' y 'logout' desde useAuth
...
const handleLogout = async () => {
  await logout(); // Usamos la función logout que obtuvimos
  router.push('/login');
};

// Lógica para cerrar el dropdown si se hace click fuera
onMounted(() => {
